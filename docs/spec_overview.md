...
### 2026-07-16〜17 追加の高速化（`feature/amber-multiprocessing`）

BTBT/BTNT/DNTT の glide グリッド計算（alpha×phi×z、a-stack/b-stack）を素材に、
上記の改善後もなお残っていたボトルネックを発見・修正した。

**1. ジョブ起動遅延の解消（ダイマー入力ファイルの書き出しタイミング変更）**

`driver_gene_phi.py`/`driver_screw_phi.py` は、座標降下法の次の探索点を決める
`request_point()` の中で mol2/tleap.in ファイルを即座に書き出していた。そのため、
探索初期に `df_init` 全行（最大2160行×候補9点）ぶんの入力ファイルを生成し終える
まで、Amber ジョブが1つも起動できなかった。

実測：BTBT 12958点グリッドで最初の `.out` 出現まで **13分18秒**（全体の約8%）。

修正：`driver_stacking.py` と同じ設計に統一し、`write_dimer_inputs()` の呼び出しを
`request_point()` から `launch()` 側に移動。`job_queue` には点とダイマー種別だけを
積み、実ファイル書き出しは `num_nodes` 個の同時実行枠に入るタイミングまで遅延させた。

**2. `OMP_NUM_THREADS=1`（最大の効果、約3.9倍高速化）**

sander はデフォルトで**ノードの全コア数ぶん OpenMP スレッド**を起動しようとする。
60原子程度の小さいダイマー系1点評価にはスレッド並列化の恩恵が全くないが、
`num_nodes`（38〜50）個の sander を同時実行すると「(コア数)×(同時ジョブ数)」ぶんの
スレッドが実コア数を奪い合う大渋滞になる（例：40コアノードで40プロセス×40スレッド
=1600スレッドが40コアを取り合う）。

実測（BTNT_glide_2稼働中の本番ノードで直接検証）：
- `OMP_NUM_THREADS` 未設定：2.566秒/ダイマー
- `OMP_NUM_THREADS=1`：0.666秒/ダイマー（**約3.9倍高速化**）
- エネルギー値は完全一致（決定的な単点評価のため精度への影響なし）

参考（孤立環境での実測）：sander内部の実計算時間（`.out`の"Total time"）は約
0.02〜0.03秒、tleap単体は約0.08秒、sander単体は約0.095秒。孤立環境では計算自体は
一瞬であり、実際の律速は「大量の同時プロセスによるスレッド競合」だった。

修正：ジョブスクリプトに `export OMP_NUM_THREADS=1`（念のため `MKL_NUM_THREADS=1`,
`OPENBLAS_NUM_THREADS=1` も）を追加。`driver_gene_phi.py`, `driver_screw_phi.py`,
`driver_stacking.py`, `eval_vdw_grid.py` のジョブスクリプト生成箇所すべてに適用。

**3. `run.py` パイプラインから不要な eval 呼び出しを削除**

`run.py` の `vdw` ステップが、VdW スウィープ直後に自動で `job_eval_grid.py`
（グリッド全点の単点エネルギー評価）を実行していたが、Amber 最適化本体
（`driver_gene_phi.py`）は既存の `E` 列を一切参照しない（`status` を無条件で
`InProgress` にリセットする）ため、この事前評価は最適化には無関係かつ計算資源の
無駄だった。加えて、この評価ステップの出力が `step1_init_params.csv` を上書きする
際、下記4の「stale split」バグの主要な発生源にもなっていた。

修正：`run.py` のパイプラインから eval 呼び出しを削除。`job_eval_grid.py` 自体は
残し、Tab0（Streamlit UI）の「お手本」にオプション手順として追記（E値付き
プレビューが見たい場合のみ手動実行）。

**4. 古い分割ディレクトリの自動無効化（フィンガープリント方式）**

同じ `auto_dir` を異なるパラメータ（`run_config.yaml` 変更後）で再実行すると、
`split_*`/`eval_split_*` ディレクトリが「既に存在する」というだけの理由で無条件に
再利用され、古いパラメータの計算結果が新しい計算に混入していた。

修正：`cluster.py` に `invalidate_stale_splits()` を追加。入力 CSV のキー列
（`status`/`E` 除く）から SHA256 ハッシュを計算し、前回実行時の値と比較。異なれば
古い分割ディレクトリを自動削除してから再計算する。`job_phi.py`, `job_screw_phi.py`,
`job_eval_grid.py`, `job_stacking.py` に適用。

**5. 複数モノマー同時実行時のジョブ完了待ちバグ**

BTNT・DNTT のように複数モノマーのパイプラインを同時に流すと、
`get_my_job_count()` が `qstat -u <user>` の全ジョブを無条件にカウントしていたため、
片方（BTNT）の全 split が完了していても、もう片方（DNTT）のジョブが残っている限り
`run.py` の「collect待ち」ループが終わらず、結果マージが止まったままになった。

修正：`get_my_job_count()` に `name_prefix` 引数を追加し、ジョブ名
（`f"{monomer}_split{i}"` 等）で絞り込めるようにした。`wait_for_free_node()` 内の
呼び出し（ユーザー全体の同時実行数の上限判定）は意図通りスコープなしのまま維持。

**6. Tab1（精細スキャン設定）の予想計算時間の見積もりバグ**

予想時間の計算式が「並列ノード数」（＝SGE への分割投入数、例：6）だけで割って
いたが、実際は1分割の中でさらに SGE の空きコア数ぶん（38〜50）Amber が同時実行
される。真の並列度は「分割数×1分割あたりコア数≈264」であり、分割数だけで割ると
桁違いに過大評価（実例：1133分という表示）になっていた。加えて `vdw_select` で
a-stack/b-stack を両方選択すると実際のグリッド行数が選択数倍になる点も未反映
だった。

修正：分割数×平均コア数（44、gr1.q/gr2.qの平均）で割るよう修正。vdw_select の
選択数ぶんの倍率も追加。

**検討したが採用しなかった案：** `multiprocessing.Pool` 移行（tleap/sander を
OS プロセスとして都度起動するのをやめ、フォースフィールドパラメータを使い回す
ワーカープロセス方式にする）は当初の本命案だったが、`OMP_NUM_THREADS=1` 修正
だけで実測3.9倍という大きな効果が出たため、より大規模なリスク・実装コストを
伴うこちらは現時点では見送った。
