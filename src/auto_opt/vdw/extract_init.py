...
def extract_init(vdw_csv: str, out_csv: str, round_ab: int = 1, minima: bool = False) -> None:
    ...
    for (z, alpha), g in df.groupby(["z", "alpha"], sort=False):
    ...
    out_df = out_df.drop_duplicates().sort_values(["z", "alpha", "a", "b"]).reset_index(drop=True)
