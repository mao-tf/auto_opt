...
# --- 幾何 --------------------------------------------------------------------
def t_shaped_pair(base_axyz, Rz, z):
    axyz_1, axyz_2 = [], []
...
def sweep(monomer_path: str, out_dir: str, z_max: float, z_step: float,
    ...
    # R_a, R_b は alpha のみの関数 → 先にメモ化
    RaRb = {}
    for alpha in alphas:
        Rz = Rod(ez, alpha)
        axyz_1, _ = t_shaped_pair(base_axyz, Rz, z=0.0)
        Ra = vdw_R(axyz_1, axyz_1, 0.0)
        Rb = vdw_R(axyz_1, axyz_1, 90.0)
        RaRb[alpha] = (Ra, Rb)
    ...
    for z in z_vals:
        for alpha in alphas:
            Rz = Rod(ez, alpha)
            axyz_1, axyz_2 = t_shaped_pair(base_axyz, Rz, z)
            R_a, R_b = RaRb[alpha]
            for beta in thetas:
