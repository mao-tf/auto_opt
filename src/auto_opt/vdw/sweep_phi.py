...
Usage Example:
1. Range mode (両タイプ出力):
   python -m auto_opt.vdw.sweep_phi --monomer-path data/monomer/DNTT.xyz --out-dir runs/ \
       --z-min 0 --z-max 3 --z-step 0.5 \
       --alpha-min 60 --alpha-max 70 --alpha-step 5 \
       --phi-min 0 --phi-max 10 --phi-step 5

2. List mode (alpha):
   python -m auto_opt.vdw.sweep_phi ... --alpha-list 60 65 70

3. a-stack のみ出力:
   python -m auto_opt.vdw.sweep_phi ... --select a-stack

4. b-stack のみ出力:
   python -m auto_opt.vdw.sweep_phi ... --select b-stack
"""
...
def t_shaped_pair(base_axyz, Rx, Rz, a, b, z):
    axyz_1, axyz_2 = [], []
...
def sweep(monomer_path: str, out_dir: str,
    ...
    total_iter = len(z_vals) * len(alphas) * len(phis)
        ...
        for alpha in alphas:
            ...
            for phi in phis:
                ...
                if accept_all or "b-stack" in accept_set:
                    ...
                    axyz_c, axyz_t = t_shaped_pair(base_axyz, Rx, Rz, 0, R_b, z)
                ...
                if accept_all or "a-stack" in accept_set:
                    ...
                    axyz_c, axyz_t = t_shaped_pair(base_axyz, Rx, Rz, R_a, 0, z)
    ...
    df = df.sort_values(['z', 'alpha', 'phi']).reset_index(drop=True)
...
# --- CLI ---------------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="vdW sweep for glide+phi")
