...
from auto_opt.stacking.make_io_stacking_screw_phi import get_14_pairs_xyzR, get_xyzR_lines
...
def exec_amber_job(auto_dir, monomer_name, params_dict, isTest=False):
    ...
    phi_str  = f"{params_dict['phi']:.1f}".replace('.', 'p').replace('-', 'm')
    ...
    base_file_name = f"{monomer_name}_beta{beta_str}_z{z_str}_phi{phi_str}_cx{cx_str}_cy{cy_str}_cz{cz_str}"
...
def main_process(args):
    ...
    fixed_keys = ['a', 'bt1', 'bt2', 'b', 'z', 'beta', 'alpha1', 'alpha2', 'phi']

    print("初期タスクのセットアップ中...")
    for _, row in df_init.iterrows():
        beta  = np.round(row['beta'],  1)
        z_vdw = np.round(row['z'],     1)
        phi   = np.round(row['phi'],   1)
        cx    = np.round(row['cx'],    1)
        cy    = np.round(row['cy'],    1)
        cz_vdw= np.round(row['cz'],   1)

        group_key = (beta, z_vdw, phi, cx, cy)
    ...
    while True:
        ...
        for file_name, task in running_jobs.items():
            ...
            if len(E_list) == 13:
                E_total = (E_list[0] + E_list[1] + E_list[2] + E_list[3] + E_list[4]
                           + (E_list[5] + E_list[6] + E_list[7] + E_list[8]
                              + E_list[9] + E_list[10] + E_list[11] + E_list[12]) / 2)
                beta  = np.round(task['beta'], 1)
                z_vdw = np.round(task['z'],    1)
                phi   = np.round(task['phi'],  1)
        ...
        for group_key, known_czs in completed_czs.items():
            ...
            if not needs_down and not needs_up:
                ...
                beta_k, z_k, phi_k, cx_k, cy_k = group_key
                mask = (
                    (np.round(df_init['beta'], 1) == beta_k) &
                    (np.round(df_init['z'],    1) == z_k)    &
                    (np.round(df_init['phi'],  1) == phi_k)  &
                    (np.round(df_init['cx'],   1) == cx_k)   &
                    (np.round(df_init['cy'],   1) == cy_k)
                )
                task_base = {k: v for k, v in df_init.loc[mask].iloc[0].to_dict().items()
                             if k in fixed_keys}

                if needs_down and cz_down not in queued_or_running_czs[group_key]:
                    job_queue.append({'cx': cx_k, 'cy': cy_k, 'cz': cz_down, **task_base})
                    queued_or_running_czs[group_key].add(cz_down)

                if needs_up and cz_up not in queued_or_running_czs[group_key]:
                    job_queue.append({'cx': cx_k, 'cy': cy_k, 'cz': cz_up, **task_base})
                    queued_or_running_czs[group_key].add(cz_up)

        while len(running_jobs) < args.num_nodes and job_queue:
        ...
        if current_time - last_save_time > 10.0 and results_list:
            pd.DataFrame(results_list).to_csv(auto_csv_path, index=False)
            last_save_time = current_time
            done_groups = sum(1 for s in group_status.values() if s == 'Done')
            print(f"[{time.strftime('%H:%M:%S')}] 進捗: {done_groups}/{len(group_status)} 箇所で谷底確定."
                  f" (実行中: {len(running_jobs)}個, キュー待機: {len(job_queue)}個)")

        if all(s == 'Done' for s in group_status.values()) and not running_jobs and not job_queue:
            if results_list:
                pd.DataFrame(results_list).to_csv(auto_csv_path, index=False)
            print("すべての探索が完了しました！")
            break
