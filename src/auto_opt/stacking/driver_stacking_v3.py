...
def main_process(args):
    ...
    # --- メモリ上での状態管理 ---
    job_queue = []               # 投入待ちのタスクリスト
    running_jobs = {}            # 実行中: {file_name: task_dict}
    completed_czs = {}           # (cx, cy) -> {cz: E_total}
    queued_or_running_czs = {}   # (cx, cy) -> set(cz) (重複投入防止用)
    results_list = []            # 完了した全結果を貯めるリスト
    group_status = {}            # (cx, cy) -> 'Working' or 'Done'

    fixed_keys = ['a', 'b', 'z', 'phi', 'alpha1', 'alpha2']
    
    print("🚀 初期タスクのセットアップ中...")
    # 全ての (cx, cy) に対して、vdWの高さ(cz)を中心に ±0.2 の5点を「先読み」でキューに突っ込む
    for _, row in df_init.iterrows():
        cx, cy, cz_vdw = np.round(row['cx'], 1), np.round(row['cy'], 1), np.round(row['cz'], 1)
        group_key = (cx, cy)
    ...
    while True:
        ...
        for file_name, task in running_jobs.items():
            ...
            if len(E_list) == 14:
                # 計算完了！
                E_total = sum(E_list)
                cx, cy, cz = task['cx'], task['cy'], task['cz']
