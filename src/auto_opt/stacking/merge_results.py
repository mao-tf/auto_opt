...
def merge_csvs(auto_dir: str, monomer_name: str | None = None,
    ...
    if monomer_name and 'E' in merged.columns:
        ...
        if e_mono_list:
            ...
            print(f"E_int = E - {_N_MONO_EFF} × {e_mono:.3f}")
        else:
            print(f"[警告] {ref_path} からエネルギーを読み取れませんでした。E_int は計算しません。")
    ...
    if 'E_int' in merged.columns:
        best = merged.loc[merged['E_int'].idxmin()]
        print(f"最安定: E_int = {best['E_int']:.3f} kcal/mol  (cy={best.get('cy', '?')}, cz={best.get('cz', '?')})")
...
if __name__ == '__main__':
    ...
    ap.add_argument('--monomer-name', default=None,  help='モノマー名（E_int 計算に使用）')
    ap.add_argument('--data-dir',     default=None,  help='data/ ディレクトリのパス（省略時はパッケージ付属）')
    args = ap.parse_args()

    data_dir = Path(args.data_dir) if args.data_dir else None
    merge_csvs(args.auto_dir, monomer_name=args.monomer_name, data_dir=data_dir)
