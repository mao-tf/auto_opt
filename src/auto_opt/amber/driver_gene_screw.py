...
import pandas as pd
import time
from auto_opt.amber.make_io_gene_screw import exec_gjf ##計算した点のxyzfileを出す
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx, row in df_prg_4.iterrows():
        ...
        if len(E_list4) != 1:
            ...
            len_prg_4 -= 1
            E4 = float(E_list4[0]) -2*E_mono # Updated to E4
            E4=np.round(E4,4)
            df_E_4.loc[idx, ['E4', 'status']] = [round(E4,4), 'Done']
            df_E_4.to_csv(auto_csv_4, index=False)  # Updated to auto_csv_4
              #  after one iteration
