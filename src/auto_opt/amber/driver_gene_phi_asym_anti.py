...
import pandas as pd
import time
from auto_opt.amber.make_io_gene_phi_asym_anti import exec_gjf ##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['alpha','phi','a','b','z','E','E1','E2','E3','E4','status'])##いじる
    ...
    if not os.path.exists(auto_csv_path4):        
        df_E_4 = pd.DataFrame(columns = ['alpha','phi','a','b','z','E4','status','file_name'])##いじる
        df_E_4.to_csv(auto_csv_path4,index=False)##step3を二段階でやる場合二段階目ではinitをやらないので念のためmainにも組み込んでおく
