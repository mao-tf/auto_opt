...
def _prepare_amber_resources(auto_dir: str):
    ...
    # frcmod（使うなら）
    for name in ["epsilon_p.frcmod", "epsilon_t_3.frcmod"]:
        s = RES / name
        if s.exists():
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx, row in df_prg_1.iterrows():
        ...
        if len(E_list1)!=1 :##get Eの長さは計算した分子の数
            ...
            len_prg_1-=1
            E1=float(E_list1[0])-2*E_mono_p##8分子に向けてep1,ep2作成　ep1:b ep2:a
            E1=np.round(E1,4)
    ...
    for idx, row in df_prg_2.iterrows():
        ...
        if len(E_list2) != 1:
            ...
            len_prg_2 -= 1
            E2 = float(E_list2[0]) -2*E_mono_p  # Updated to E2
            E2=np.round(E2,4)
    ...
    for idx, row in df_prg_3.iterrows():
        ...
        if len(E_list3) != 1:
            ...
            len_prg_3 -= 1
            E3 = float(E_list3[0]) -2*E_mono_t # Updated to E3
            E3=np.round(E3,4)
